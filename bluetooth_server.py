...
class StepperController:
    def __init__(self) -> None:
        if pigpio is None:
            raise RuntimeError("pigpio library is required and could not be imported")

        self.pi = pigpio.pi()
        if not self.pi.connected:
            raise RuntimeError("pigpio daemon is not connected; aborting")

        self.pi.set_mode(STEPPER_DIRECTION_PIN, pigpio.OUTPUT)
        self.pi.set_mode(STEPPER_STEP_PIN, pigpio.OUTPUT)
    ...
    def rotate(self, angle_degrees: float, clockwise: bool) -> None:
        ...
        self.pi.write(STEPPER_DIRECTION_PIN, 1 if clockwise else 0)
        for _ in range(steps):
            self.pi.write(STEPPER_STEP_PIN, 1)
            time.sleep(STEPPER_PULSE_HIGH_SECONDS)
            self.pi.write(STEPPER_STEP_PIN, 0)
