...
- `scripts/run_video_server.py` → `rov/video_server.py` (Flask camera server with `/`, `/status`, `/video/<camera>`).
...
- Video `/status` includes service uptime and per-camera online/failure details.
