...
CameraBasis = Tuple[List[float], List[float], List[float], List[float]]
...
def camera_basis(camera: Camera) -> CameraBasis:
    forward = [
...
def world_to_screen(point_world: Sequence[float], camera: Camera, basis: Optional[CameraBasis] = None) -> Optional[Tuple[int, int]]:
    ...
    view_x = vec_dot(relative_point, right)
    view_y = vec_dot(relative_point, up)
    view_z = vec_dot(relative_point, forward)

    if view_z <= CAMERA_NEAR_PLANE or not all(math.isfinite(v) for v in (view_x, view_y, view_z)):
        return None

    x = view_x * CAMERA_FOCAL_LENGTH / view_z + WINDOW_SIZE[0] / 2
    y = -view_y * CAMERA_FOCAL_LENGTH / view_z + WINDOW_SIZE[1] / 2
...
def draw_rov(screen: pygame.Surface, sim: ROVSimulator, thrusters: List[float], camera: Camera):
    ...
    projected = [world_to_screen(v, camera, basis) for v in world_vertices]

    for edge in edges:
        start = projected[edge[0]]
        end = projected[edge[1]]
        if start is not None and end is not None:
            pygame.draw.line(screen, (185, 220, 255), start, end, 2)
    ...
    front_px = world_to_screen(front_center_world, camera, basis)
    back_px = world_to_screen(back_center_world, camera, basis)
    ...
    for index, thruster in enumerate(THRUSTERS):
        ...
        center_px = world_to_screen(center_world, camera, basis)
        ...
        start_px = world_to_screen(axis_start_world, camera, basis)
        end_px = world_to_screen(axis_end_world, camera, basis)
        radius = 5
        ...
        thrust_tip_px = world_to_screen(thrust_tip_world, camera, basis)
        if thrust_tip_px is not None:
            pygame.draw.line(screen, color, center_px, thrust_tip_px, 4)
