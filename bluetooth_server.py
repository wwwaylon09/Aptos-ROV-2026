...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--no-register-sdp",
        action="store_true",
        help="Disable SDP registration (not recommended for Windows COM interoperability).",
    )
    parser.add_argument(
        "--allow-no-sdp",
        action="store_true",
        help="Continue even if SDP registration fails.",
    )
    parser.add_argument(
        "--sdp-name",
...
def run_server(adapter: str, channel: int, buffer_size: int, echo: bool, register_sdp: bool, allow_no_sdp: bool, sdp_name: str) -> None:
    ...
    if register_sdp:
        ...
        if not sdp_ok and not allow_no_sdp:
            server_sock.close()
            raise RuntimeError(
                "SDP registration failed. Refusing to continue because Windows COM mapping is likely incorrect. "
                "Install/enable bluez sdptool or run with --allow-no-sdp to override."
            )
    else:
        if not allow_no_sdp:
            server_sock.close()
            raise RuntimeError(
                "SDP registration is disabled. This often causes Windows to connect to the wrong service/channel. "
                "Use default SDP registration or pass --allow-no-sdp to override."
            )
        print("[WARN] SDP registration disabled by request; continuing with potential COM/channel mismatch risk.")
...
def main() -> None:
    ...
    try:
        run_server(
            ...
            echo=args.echo,
            register_sdp=not args.no_register_sdp,
            allow_no_sdp=args.allow_no_sdp,
